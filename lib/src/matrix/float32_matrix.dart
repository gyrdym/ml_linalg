...
class Float32Matrix
  ...
  Matrix transpose() {
    ...
    return Float32Matrix.fromFlattenedList(transposed, columnCount, rowCount);
  }
  ...
  Matrix mapElements(double Function(double element) mapper) {
    ...
    return Float32Matrix.fromFlattenedList(mapped, rowCount, columnCount);
  }
  ...
  @override
  Matrix mapRows(Vector Function(Vector row) mapper) => Float32Matrix.fromRows(
  ...
  Matrix uniqueRows() {
    ...
    return Float32Matrix.fromRows(checked);
  }
  ...
  Vector _columnMean() {
    final means = Float32List(columnCount);
    var j = 0;
  ...
  Matrix insertColumns(int targetIndex, List<Vector> columns) {
    ...
    return Float32Matrix.fromColumns(newColumns);
  }
  ...
  Matrix sort(double Function(Vector vector) selectSortValue,
    ...
    switch (axis) {
      case Axis.rows:
        return Float32Matrix.fromRows(doSort(rows));

      case Axis.columns:
        return Float32Matrix.fromColumns(doSort(columns));
  ...
  Matrix pow(num exponent) {
    ...
    return Float32Matrix.fromFlattenedList(result, rowCount, columnCount);
  }
  ...
  Matrix _forwardSubstitutionInverse() {
    ...
    return Float32Matrix.fromFlattenedList(X, rowCount, rowCount);
  }
  ...
  Iterable<Matrix> _choleskyDecomposition() {
    ...
    return [
      Float32Matrix.fromFlattenedList(L, rowCount, rowCount),
      Float32Matrix.fromFlattenedList(U, rowCount, rowCount)
    ];
  }
  ...
  Iterable<Matrix> _qrDecomposition() {
    ...
    final R = Float32List.fromList(_flattenedList);
    
    // Q will be built up as a product of Householder reflections
    // Start with identity
    final Q = Float32List(rowCount * rowCount);
    for (var i = 0; i < rowCount; i++) {
      Q[i * rowCount + i] = 1.0;
    }
    ...
    for (var k = 0; k < columnCount; k++) {
      ...
      final v = Float32List(rowCount - k);
    ...
    final qMatrix = Float32Matrix.fromFlattenedList(Q, rowCount, rowCount);
    final rMatrix = Float32Matrix.fromFlattenedList(R, rowCount, columnCount);

    return [qMatrix, rMatrix];
  }

  double _computeColumnNorm(Float32List matrix, int col, int startRow) {
    var sum = 0.0;
  ...
  void _applyHouseholderToR(Float32List R, Float32List v, int k) {
  ...
  void _applyHouseholderToQ(Float32List Q, Float32List v, int k) {
    // For each row of Q
    for (var i = 0; i < rowCount; i++) {
  ...
  num _rayleighQuotient(Vector eigenVector) {
    return (Float32Matrix.fromRows([eigenVector]) * this)
            .toVector()
            .dot(eigenVector) /
  ...
  Matrix _matrixByVectorMult(Vector vector) {
    ...
    return Float32Matrix.fromColumns([vectorColumn]);
  }
