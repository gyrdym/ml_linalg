...
class Float32x4Vector extends Object with
  ...
  Float32x4Vector(int length, [this.type = MLVectorType.column]) {
    this.length = length;
    data = createSIMDList(length);
  }

  /// Creates a vector from collection
  Float32x4Vector.from(Iterable<double> source, [this.type = MLVectorType.column]) {
  ...
  Float32x4Vector.fromSIMDList(Float32x4List source, [int origLength, this.type = MLVectorType.column]) {
  ...
  Float32x4Vector.filled(int length, double value, [this.type = MLVectorType.column]) {
    this.length = length;
  ...
  Float32x4Vector.zero(int length, [this.type = MLVectorType.column]) {
    this.length = length;
  ...
  Float32x4Vector.randomFilled(int length, {int seed, this.type = MLVectorType.column}) {
    this.length = length;
